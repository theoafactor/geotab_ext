...
def generate_header_file(table_name, bucket_name, folder_name, geotab_dir):
    try:
        ...
        print(f"The data_file_path: {data_file_path}")
    ...
    try:
        ...
        for table in active_tables:
            ...
            if max_records_per_call == 0:
                print(f"Fetching all data non-paginable_entities: {table_name}")
                ...
                if data:
                    ...
                    print(f"No data found for non-paginable_entities {table_name}")
                continue
            ...
            while True:
                ...
                print('calling fetch data', client, type_name, max_records_per_call, load_type, offset, last_id, from_date)
                print("Client: ", client)
                print("type_name: ", type_name)
                print("max_records_per_call: ", max_records_per_call)
                ...
                print("Data fetched before storage: ", data)
                print(len(data))
                if not data:
                    print(f"No more data to fetch for {table_name}.")
                    break

                # Store data in S3
                print("storing data ... within loop")
                store_data_in_s3(data, table_name, bucket_name, folder_name)

                # Update pagination parameters
                if type_name in paginable_entities:
                    last_id = data[-1].get("id")
                    offset = data[-1].get(type_to_sortkey[type_name])
